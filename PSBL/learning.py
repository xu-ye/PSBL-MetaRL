...
from PSBL import utils
#from PSBL.agent import AgentOnlineAllTstep
from PSBL.envs.env_utils import (
    ReturnHistory,
    SuccessHistory,
    ExplorationWrapper,
    SequenceWrapper,
...
class Experiment:
    ...
    memory_len:int=150
    agent_network:bool=1

    # Logging
    log_to_wandb: bool = False
    wandb_project: str = os.environ.get("PSBL_WANDB_PROJECT")
    wandb_entity: str = os.environ.get("PSBL_WANDB_ENTITY")
    ...
    train_grad_updates_per_epoch: int = 2000
    val_interval: int = 2
    val_timesteps_per_epoch: int = 10000
    val_checks_per_epoch: int = 20
    ...
    critic_loss_weight: float = 10
    actor_loss_weight: float = 1
    sl_loss_weight: float = 0.1
    ...
    weighted_sample:bool=0
    agent_new:bool=1
    episode_emb:bool=0
    
    #agent
    sl_rewads_coef:float=0.8,
    sl_obs_coef:float=0.8,
    r_bias_coef:float=3,
    ...
    def load_checkpoint(self, epoch: int = None, loading_best: bool = False):
        ...
        ckpt = torch.load(os.path.join(self.ckpt_dir, ckpt_name),map_location=self.DEVICE)
        #ckpt = torch.load(os.path.join(self.ckpt_dir, ckpt_name),map_location=cpu)
        ...
        torch.save(self.policy,'/home/xty/code/TransformerOnline/examples/model/HalfCheetah-Vel_policy.pt')
        1
        
    def load_data_state(self, path):
        ...
        self.grad_scaler.load_state_dict(ckpt["grad_scaler"])
        self.best_return = ckpt["best_return"]
        #torch.save(self.policy,'/home/xty/code/PSBL_MetaRL/examples/model/Halfcheetah_policy.pt')
        1
    ...
    def init_dsets(self):
        ...
        self.train_dset = TrajDset(
            ...
            dset_name=self.dset_name,
            dset_split="train",
            items_per_epoch=self.train_grad_updates_per_epoch * self.batch_size,# 1000*24
    ...
    def init_dloaders(self):
        ...
        if self.weighted_sample:
            self.train_dset.refresh_files_weighted()
            self.val_dset.refresh_files_weighted()
        else:
            self.train_dset.refresh_files()
            self.val_dset.refresh_files()
        ...
        self.val_dloader = DataLoader(
            self.val_dset,
            batch_size=self.batch_size,
            num_workers=self.dloader_workers,#8
            collate_fn=RLData_pad_collate,
            pin_memory=True,
            #shuffle=True,
        )
    ...
    def init_logger(self):
        ...
        if self.log_to_wandb:
            ...
            wandb.define_metric("total_frames")
        # set all other train/ metrics to use this step
            #wandb.define_metric(f"{key}/*", step_metric="total_frames")
            wandb.define_metric("val/*", step_metric="total_frames")
            wandb.define_metric("*", step_metric="total_frames")
    ...
    def init_model(self):
        ...
        policy_kwargs = {
            ...
            "episode_emb":self.episode_emb,
            "sl_rewads_coef":self.sl_rewads_coef,
            "r_bias_coef":self.r_bias_coef,
            "sl_obs_coef":self.sl_obs_coef,
            
  
        }
        #self.policy = Agent(**policy_kwargs)
        self.policy = self.policy_Cls(**policy_kwargs)
    ...
    def interact(
        ...
        if buffers is None:
            ...
            envs.reset()
            make_buffer = partial(
                GPUSequenceBuffer, self.DEVICE, self.max_seq_len, self.parallel_actors,self.episode_length,self.horizon,
            )
        ...
        def get_t(_dones=None):
            ...
            obs_seqs.add_timestep(_obs, _dones)# 将12个actor的观测分别存入
        ...
        for step in iter_:
            obs_tc_t = obs_seqs.sequences#[12,1,6]
    ...
    def interact_test(
        ...
        if buffers is None:
            ...
            envs.reset()
            make_buffer = partial(
                GPUSequenceBuffer, self.DEVICE, self.memory_len, self.parallel_actors,self.episode_length,self.horizon,
            )
            obs_seqs = make_buffer()
            goal_seqs = make_buffer()
            rl2_seqs = make_buffer()
        ...
        def get_t1(_dones=None):
            ...
            obs_seqs.add_timestep_test(_obs, _dones)# 将12个actor的观测分别存入
            goal_seqs.add_timestep_test(_goal, _dones)
            rl2_seqs.add_timestep_test(_rl2, _dones)
        ...
        for step in iter_:
            if new_embedding:   
                obs_tc_t = obs_seqs.sequences_test2()#[12,1,6]
                goals_tc_t = goal_seqs.sequences_test2()
                rl2_tc_t = rl2_seqs.sequences_test2()
                time_idxs = obs_seqs.time_idxs_test2
            else:
                obs_tc_t = obs_seqs.sequences_test()#[12,1,6]
                goals_tc_t = goal_seqs.sequences_test()
                rl2_tc_t = rl2_seqs.sequences_test_rl2s()
                if self.episode_emb:
                    
                    time_idxs = obs_seqs.time_idxs_test_episode
                else:
                    time_idxs = obs_seqs.time_idxs_test
            seq_lengths = obs_seqs.sequence_lengths
            #time_idxs = obs_seqs.time_idxs_test
            #num=( step-self.memory_len-1)//episode_length+1
            ...
            #done1=np.zeros_like(done)
            
            get_t1(done)
            hidden_state = self.policy.traj_encoder.reset_hidden_state(
                hidden_state, done
            )
            ext_rew_all.append(ext_rew.reshape((ext_rew.shape[0],1)))
            ext_rew.reshape((1,ext_rew.shape[0]))

            if render:
                envs.render()
        #episode_length=50
        rewards=np.concatenate(ext_rew_all,axis=1)
        num_episode=int(rewards.shape[1]/episode_length)
        rewards_new=np.zeros((rewards.shape[0],num_episode))
        for i in range(num_episode):
            rewards_new[:,i]=rewards[:,i*episode_length:(i+1)*episode_length].sum(axis=1)
        rewards_mean=rewards_new.mean(axis=0)
    ...
    def interact_test_grid(
        ...
        if buffers is None:
            ...
            envs.reset()
            make_buffer = partial(
                GPUSequenceBuffer, self.DEVICE, self.memory_len, self.parallel_actors,self.episode_length-1,self.horizon,
            )
            obs_seqs = make_buffer()
            goal_seqs = make_buffer()
            rl2_seqs = make_buffer()
    ...
    def collect_new_training_data_agent(self):
        if self.train_timesteps_per_epoch > 0:
            self.train_buffers, self.hidden_state, returns, successes,_,_ = self.interact_test(
                self.train_envs,
                self.train_timesteps_per_epoch,
                buffers=self.train_buffers,
            )
    ...
    def evaluate_val_agent(self):
        if self.val_timesteps_per_epoch > 0:
            *_, returns, successes,_,_ = self.interact_test(
                self.val_envs,
                self.val_timesteps_per_epoch,
            )
            logs = self.policy_metrics(returns, successes)
    ...
    def evaluate_test_episode(
        self, make_test_env: callable, timesteps: int, render: bool = False,episode_length:int=200,traj_length:int=800
    ):
        num_episode=int(timesteps/self.traj_save_len)
        #episode_length=50
        #traj_length=800
        horizon=402
        make = lambda: SequenceWrapper(
        ...
        iter=range(timesteps)
        for step in iter:
            *_, returns, successes,rewards,rewards_new = self.interact_test(
                test_envs,
                traj_length,
                render=render,
    ...
    def evaluate_test_episode_grid(
        self, make_test_env: callable, timesteps: int, render: bool = False,episode_length:int=200,traj_length:int=800
    ):
        num_episode=int(timesteps/self.traj_save_len)
        #episode_length=50
        ...
        iter=range(timesteps)
        for step in iter:
            *_, returns, successes,rewards,rewards_new = self.interact_test_grid(
                test_envs,
                traj_length,
                render=render,
                episode_length=episode_length,
    ...
    def compute_loss(self, batch: Batch, log_step: bool):
        ...
        sl_state_mask = state_mask[:, 1:, ...].clone()

        masked_actor_loss = (actor_state_mask * actor_loss).sum() / actor_state_mask.sum()
        
    
        if isinstance(critic_loss, torch.Tensor):
            masked_critic_loss = (critic_state_mask * critic_loss).sum() / critic_state_mask.sum()
    ...
    def compute_loss_online(self, batch: Batch, log_step: bool,step=0):
        batch.to(self.DEVICE)
        with self.caster():
            if self.agent_network:
                critic_loss, actor_loss,sl_loss,incontext_loss = self.policy.forward_agent(batch, log_step=log_step,step=step)
            else:

                critic_loss, actor_loss,sl_loss,incontext_loss = self.policy(batch, log_step=log_step,step=step)
        ...
        if self.agent_new:
            ...
            critic_state_mask = repeat(state_mask[:, 1:-1, ...], f"B L 1 -> B L {C} {G} 1")
            actor_state_mask = repeat(state_mask[:, :-2, ...], f"B L 1 -> B L {G} 1")
        sl_state_mask = state_mask[:, 1:, ...].clone()

        masked_actor_loss = (actor_state_mask * actor_loss).sum() / actor_state_mask.sum()
        #masked_sl_loss = (sl_state_mask * sl_loss).sum() / actor_state_mask.sum()
        masked_sl_loss=sl_loss
        #masked_critic_loss=critic_loss.mean()
        masked_actor_loss +=incontext_loss
        optimize_critic = True
    ...
    def train_step(self, batch: Batch, log_step: bool):
        ...
        self.grad_scaler.scale(self.actor_loss_weight*l["actor_loss"]).backward(retain_graph=True)
        #self.grad_scaler.scale(self.sl_loss_weight *l["sl_loss"]).backward(retain_graph=True)
        self.policy.critics.zero_grad(set_to_none=True)
        l["optimize_critic"]=True
        if l["optimize_critic"]:
            self.grad_scaler.scale(
                self.critic_loss_weight * l["critic_loss"]
            ).backward()
        
        l["critic_loss"]=l["critic_loss"].item()
        l["actor_loss"]=l["actor_loss"].item()
        #l["sl_loss"]=l["sl_loss"].item()
        ...
        self.optimizer.zero_grad()
        self.policy.traj_encoder.zero_grad()
        self.policy.tstep_encoder.zero_grad()
        return l

    def train_step_online(self, batch: Batch, log_step: bool,step):
        """
        See a simplified example of how this changes the classic
        actor-critic update to support a shared sequnce model
        optimized on the actor and critic losses simultaneously
        ...
        https://colab.research.google.com/drive/1XO8MNd_DNArGyoAbroXoQeGblv7Jb9ND#scrollTo=xQe5yry0b6Hl
        """
        l = self.compute_loss_online(batch, log_step=log_step,step=step)
        self.optimizer.zero_grad()
        if step % 1==0:
            self.grad_scaler.scale(self.actor_loss_weight*l["actor_loss"]).backward(retain_graph=True)
        #self.grad_scaler.scale(self.sl_loss_weight *l["sl_loss"]).backward(retain_graph=True)
        sl_loss=self.sl_loss_weight *l["sl_loss"]
        sl_loss.backward(retain_graph=True)
        self.policy.critics.zero_grad(set_to_none=True)
        l["optimize_critic"]=True
        if l["optimize_critic"]:
            self.grad_scaler.scale(
    ...
    def train_step_new(self, batch: Batch, log_step: bool,train_step):
        """
        See a simplified example of how this changes the classic
        actor-critic update to support a shared sequnce model
        optimized on the actor and critic losses simultaneously
        ...
        self.optimizer.zero_grad()
        loss=l["actor_loss"]+self.sl_loss_weight *l["sl_loss"]+self.critic_loss_weight * l["critic_loss"]
        #loss=self.sl_loss_weight *l["sl_loss"]
    ...
    def train_step_new2(self, batch: Batch, log_step: bool,train_step):
        """
        See a simplified example of how this changes the classic
        actor-critic update to support a shared sequnce model
        optimized on the actor and critic losses simultaneously
        ...
        self.policy.critics.zero_grad(set_to_none=True)
        #if l["optimize_critic"]:
        #    self.grad_scaler.scale(self.critic_loss_weight * l["critic_loss"]).backward()
    ...
    def val_step_online(self, batch):
        with torch.no_grad():
            return self.compute_loss_online(batch, log_step=True)
    ...
    def learn(self):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            if no_interact:
                ...
                if self.agent_network:
                    if epoch % self.val_interval == 0:
                        self.evaluate_val_agent()
                    1
                    self.collect_new_training_data_agent()
                else:
            ...
            for train_step, batch in make_pbar(self.train_dloader, True, epoch):
                ...
                if self.on_line:
                    loss_dict = self.train_step_online(batch, log_step=log_step,step=train_step)
                else:
                    loss_dict = self.train_step(batch, log_step=log_step)
                #loss_dict = self.train_step_new(batch, log_step=log_step,train_step=train_step)
            ...
            if epoch % self.val_interval == 0 :
                ...
                for val_step, batch in make_pbar(self.val_dloader, False, epoch):
                    if self.on_line:
                        loss_dict = self.val_step_online(batch)
                    else:
