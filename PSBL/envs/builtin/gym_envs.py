...
class _DiscreteToBox(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=1, shape=(self.n,), dtype=np.int3264
        )

    def observation(self, obs):
        arr = np.zeros((self.n,), dtype=np.int3264)
        arr[obs] = 1
        return arr
...
class _MultiDiscreteToBox(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=1, shape=(self.categories * self.max_options,), dtype=np.int3264
        )

    def observation(self, obs):
        arr = np.zeros((self.categories, self.max_options), dtype=np.int3264)
...
class POPGymEnv(GymEnv):
    def __init__(self, env_name: str):
        try:
            ...
            msg = "Missing POPGym Install: `pip install PSBL[envs]` or `pip install popgym`"
            print(msg)
            exit()
