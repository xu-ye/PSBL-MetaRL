...
class MjrContextWrapper(object):
    ...
    @property
    def texture(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.texture, dtype=np.int32, count=(100)), (100,))
        arr.setflags(write=False)
        return arr
    ...
    def textureType(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.textureType, dtype=np.int32, count=(100)), (100,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def charWidth(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.charWidth, dtype=np.int32, count=(127)), (127,))
        arr.setflags(write=False)
        return arr
    ...
    def charWidthBig(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.charWidthBig, dtype=np.int32, count=(127)), (127,))
        arr.setflags(write=False)
        return arr
...
class MjDataWrapper(object):
    ...
    @property
    def nwarning(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.nwarning, dtype=np.int32, count=(8)), (8,))
        arr.setflags(write=False)
        return arr
    ...
    def warning_info(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.warning_info, dtype=np.int32, count=(8)), (8,))
        arr.setflags(write=False)
        return arr
    ...
    def ten_wrapadr(self):
        arr = np.reshape(
            np.fromiter(self._wrapped.contents.ten_wrapadr, dtype=np.int32, count=(self._size_src.ntendon * 1)),
    ...
    def ten_wrapnum(self):
        arr = np.reshape(
            np.fromiter(self._wrapped.contents.ten_wrapnum, dtype=np.int32, count=(self._size_src.ntendon * 1)),
            (self._size_src.ntendon, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def wrap_obj(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.wrap_obj, dtype=np.int32, count=(self._size_src.nwrap * 2)),
    ...
    @property
    def efc_type(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.efc_type, dtype=np.int32, count=(self._size_src.njmax * 1)),
    ...
    @property
    def efc_id(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.efc_id, dtype=np.int32, count=(self._size_src.njmax * 1)),
                         (self._size_src.njmax, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def efc_rownnz(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.efc_rownnz, dtype=np.int32, count=(self._size_src.njmax * 1)),
                         (self._size_src.njmax, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def efc_rowadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.efc_rowadr, dtype=np.int32, count=(self._size_src.njmax * 1)),
                         (self._size_src.njmax, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def efc_colind(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.efc_colind, dtype=np.int32,
    ...
    def efc_rownnz_T(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.efc_rownnz_T, dtype=np.int32, count=(self._size_src.nv * 1)),
                         (self._size_src.nv, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def efc_rowadr_T(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.efc_rowadr_T, dtype=np.int32, count=(self._size_src.nv * 1)),
                         (self._size_src.nv, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def efc_colind_T(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.efc_colind_T, dtype=np.int32,
...
class MjModelWrapper(object):
    ...
    def body_parentid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_parentid, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_rootid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_rootid, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_weldid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_weldid, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_mocapid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_mocapid, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_jntnum(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_jntnum, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_jntadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_jntadr, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_dofnum(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_dofnum, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_dofadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_dofadr, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_geomnum(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_geomnum, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def body_geomadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.body_geomadr, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def jnt_type(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.jnt_type, dtype=np.int32, count=(self.njnt * 1)),
                         (self.njnt, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def jnt_qposadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.jnt_qposadr, dtype=np.int32, count=(self.njnt * 1)),
                         (self.njnt, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def jnt_dofadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.jnt_dofadr, dtype=np.int32, count=(self.njnt * 1)),
                         (self.njnt, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def jnt_bodyid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.jnt_bodyid, dtype=np.int32, count=(self.njnt * 1)),
                         (self.njnt, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def dof_bodyid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.dof_bodyid, dtype=np.int32, count=(self.nv * 1)),
                         (self.nv, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def dof_jntid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.dof_jntid, dtype=np.int32, count=(self.nv * 1)),
                         (self.nv, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def dof_parentid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.dof_parentid, dtype=np.int32, count=(self.nv * 1)),
                         (self.nv, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def dof_Madr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.dof_Madr, dtype=np.int32, count=(self.nv * 1)), (self.nv, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def geom_type(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.geom_type, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def geom_contype(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.geom_contype, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def geom_conaffinity(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.geom_conaffinity, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def geom_condim(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.geom_condim, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def geom_bodyid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.geom_bodyid, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def geom_dataid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.geom_dataid, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def geom_matid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.geom_matid, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def geom_group(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.geom_group, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def site_type(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.site_type, dtype=np.int32, count=(self.nsite * 1)),
                         (self.nsite, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def site_bodyid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.site_bodyid, dtype=np.int32, count=(self.nsite * 1)),
                         (self.nsite, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def site_matid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.site_matid, dtype=np.int32, count=(self.nsite * 1)),
                         (self.nsite, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def site_group(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.site_group, dtype=np.int32, count=(self.nsite * 1)),
                         (self.nsite, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def cam_mode(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.cam_mode, dtype=np.int32, count=(self.ncam * 1)),
                         (self.ncam, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def cam_bodyid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.cam_bodyid, dtype=np.int32, count=(self.ncam * 1)),
                         (self.ncam, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def cam_targetbodyid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.cam_targetbodyid, dtype=np.int32, count=(self.ncam * 1)),
                         (self.ncam, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def light_mode(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.light_mode, dtype=np.int32, count=(self.nlight * 1)),
                         (self.nlight, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def light_bodyid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.light_bodyid, dtype=np.int32, count=(self.nlight * 1)),
                         (self.nlight, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def light_targetbodyid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.light_targetbodyid, dtype=np.int32, count=(self.nlight * 1)),
                         (self.nlight, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def mesh_faceadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.mesh_faceadr, dtype=np.int32, count=(self.nmesh * 1)),
                         (self.nmesh, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def mesh_facenum(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.mesh_facenum, dtype=np.int32, count=(self.nmesh * 1)),
                         (self.nmesh, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def mesh_vertadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.mesh_vertadr, dtype=np.int32, count=(self.nmesh * 1)),
                         (self.nmesh, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def mesh_vertnum(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.mesh_vertnum, dtype=np.int32, count=(self.nmesh * 1)),
                         (self.nmesh, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def mesh_graphadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.mesh_graphadr, dtype=np.int32, count=(self.nmesh * 1)),
                         (self.nmesh, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def mesh_face(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.mesh_face, dtype=np.int32, count=(self.nmeshface * 3)),
                         (self.nmeshface, 3,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def mesh_graph(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.mesh_graph, dtype=np.int32, count=(self.nmeshgraph * 1)),
                         (self.nmeshgraph, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def hfield_nrow(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.hfield_nrow, dtype=np.int32, count=(self.nhfield * 1)),
                         (self.nhfield, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def hfield_ncol(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.hfield_ncol, dtype=np.int32, count=(self.nhfield * 1)),
                         (self.nhfield, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def hfield_adr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.hfield_adr, dtype=np.int32, count=(self.nhfield * 1)),
                         (self.nhfield, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def tex_type(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.tex_type, dtype=np.int32, count=(self.ntex * 1)),
                         (self.ntex, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def tex_height(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.tex_height, dtype=np.int32, count=(self.ntex * 1)),
                         (self.ntex, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def tex_width(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.tex_width, dtype=np.int32, count=(self.ntex * 1)),
                         (self.ntex, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def tex_adr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.tex_adr, dtype=np.int32, count=(self.ntex * 1)),
                         (self.ntex, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def mat_texid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.mat_texid, dtype=np.int32, count=(self.nmat * 1)),
                         (self.nmat, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def pair_dim(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.pair_dim, dtype=np.int32, count=(self.npair * 1)),
                         (self.npair, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def pair_geom1(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.pair_geom1, dtype=np.int32, count=(self.npair * 1)),
                         (self.npair, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def pair_geom2(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.pair_geom2, dtype=np.int32, count=(self.npair * 1)),
                         (self.npair, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def pair_signature(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.pair_signature, dtype=np.int32, count=(self.npair * 1)),
                         (self.npair, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def exclude_signature(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.exclude_signature, dtype=np.int32, count=(self.nexclude * 1)),
                         (self.nexclude, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def eq_type(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.eq_type, dtype=np.int32, count=(self.neq * 1)),
                         (self.neq, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def eq_obj1id(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.eq_obj1id, dtype=np.int32, count=(self.neq * 1)),
                         (self.neq, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def eq_obj2id(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.eq_obj2id, dtype=np.int32, count=(self.neq * 1)),
                         (self.neq, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def tendon_adr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.tendon_adr, dtype=np.int32, count=(self.ntendon * 1)),
                         (self.ntendon, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def tendon_num(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.tendon_num, dtype=np.int32, count=(self.ntendon * 1)),
                         (self.ntendon, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def tendon_matid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.tendon_matid, dtype=np.int32, count=(self.ntendon * 1)),
                         (self.ntendon, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def wrap_type(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.wrap_type, dtype=np.int32, count=(self.nwrap * 1)),
                         (self.nwrap, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def wrap_objid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.wrap_objid, dtype=np.int32, count=(self.nwrap * 1)),
                         (self.nwrap, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def actuator_trntype(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.actuator_trntype, dtype=np.int32, count=(self.nu * 1)),
                         (self.nu, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def actuator_dyntype(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.actuator_dyntype, dtype=np.int32, count=(self.nu * 1)),
                         (self.nu, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def actuator_gaintype(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.actuator_gaintype, dtype=np.int32, count=(self.nu * 1)),
                         (self.nu, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def actuator_biastype(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.actuator_biastype, dtype=np.int32, count=(self.nu * 1)),
                         (self.nu, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def actuator_trnid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.actuator_trnid, dtype=np.int32, count=(self.nu * 2)),
                         (self.nu, 2,))
        arr.setflags(write=False)
        return arr
    ...
    def sensor_type(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.sensor_type, dtype=np.int32, count=(self.nsensor * 1)),
                         (self.nsensor, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def sensor_objid(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.sensor_objid, dtype=np.int32, count=(self.nsensor * 1)),
                         (self.nsensor, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def sensor_dim(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.sensor_dim, dtype=np.int32, count=(self.nsensor * 1)),
                         (self.nsensor, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def sensor_adr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.sensor_adr, dtype=np.int32, count=(self.nsensor * 1)),
                         (self.nsensor, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def numeric_adr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.numeric_adr, dtype=np.int32, count=(self.nnumeric * 1)),
                         (self.nnumeric, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def numeric_size(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.numeric_size, dtype=np.int32, count=(self.nnumeric * 1)),
                         (self.nnumeric, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def text_adr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.text_adr, dtype=np.int32, count=(self.ntext * 1)),
                         (self.ntext, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_bodyadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_bodyadr, dtype=np.int32, count=(self.nbody * 1)),
                         (self.nbody, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_jntadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_jntadr, dtype=np.int32, count=(self.njnt * 1)),
                         (self.njnt, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_geomadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_geomadr, dtype=np.int32, count=(self.ngeom * 1)),
                         (self.ngeom, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_siteadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_siteadr, dtype=np.int32, count=(self.nsite * 1)),
                         (self.nsite, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_camadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_camadr, dtype=np.int32, count=(self.ncam * 1)),
                         (self.ncam, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_lightadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_lightadr, dtype=np.int32, count=(self.nlight * 1)),
                         (self.nlight, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_meshadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_meshadr, dtype=np.int32, count=(self.nmesh * 1)),
                         (self.nmesh, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_hfieldadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_hfieldadr, dtype=np.int32, count=(self.nhfield * 1)),
                         (self.nhfield, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_texadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_texadr, dtype=np.int32, count=(self.ntex * 1)),
                         (self.ntex, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_matadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_matadr, dtype=np.int32, count=(self.nmat * 1)),
                         (self.nmat, 1,))
        arr.setflags(write=False)
        return arr
    ...
    @property
    def name_eqadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_eqadr, dtype=np.int32, count=(self.neq * 1)),
                         (self.neq, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_tendonadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_tendonadr, dtype=np.int32, count=(self.ntendon * 1)),
                         (self.ntendon, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_actuatoradr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_actuatoradr, dtype=np.int32, count=(self.nu * 1)),
                         (self.nu, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_sensoradr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_sensoradr, dtype=np.int32, count=(self.nsensor * 1)),
                         (self.nsensor, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_numericadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_numericadr, dtype=np.int32, count=(self.nnumeric * 1)),
                         (self.nnumeric, 1,))
        arr.setflags(write=False)
        return arr
    ...
    def name_textadr(self):
        arr = np.reshape(np.fromiter(self._wrapped.contents.name_textadr, dtype=np.int32, count=(self.ntext * 1)),
                         (self.ntext, 1,))
        arr.setflags(write=False)
        return arr
