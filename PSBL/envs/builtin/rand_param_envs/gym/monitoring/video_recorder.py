...
class TextEncoder(object):
    ...
    def capture_frame(self, frame):
        ...
        if six.b('\r') in frame_bytes:
            raise error.InvalidFrame(
                'Frame contains carriage returns (only newlines are allowed: """{}"""'.format(string))
...
class ImageEncoder(object):
    ...
    def capture_frame(self, frame):
        ...
        if frame.shape != self.frame_shape:
            raise error.InvalidFrame(
                "Your frame has shape {}, but the VideoRecorder is configured for shape {}.".format(frame.shape,
                                                                                                    self.frame_shape))
