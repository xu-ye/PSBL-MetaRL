...
from PSBL.envs.builtin.rand_param_envs.gym import benchmark_spec, error, monitoring
from PSBL.envs.builtin.rand_param_envs.gym.scoreboard.client import resource, util
...
def upload(training_dir, algorithm_id=None, writeup=None, tags=None, benchmark_id=None, api_key=None,
...
def _upload(training_dir, algorithm_id=None, writeup=None, benchmark_run_id=None, api_key=None,
            ignore_open_monitors=False):
    if not ignore_open_monitors:
        ...
        if len(open_monitors) > 0:
            ...
            raise error.Error("Still have an open monitor on {}. You must run 'env.close()' before uploading.".format(
                ', '.join(envs)))
    ...
    if logger.level <= logger.INFO:
        if training_episode_batch_id is not None and training_video_id is not None:
            logger.info('[%s] Creating evaluation object from %s with learning curve and training video', env_id,
                        training_dir)
...
(HINT: this usually means you did not yet close() your env.monitor and have not yet exited the process. You should call 'env.monitor.start(training_dir)' at the start of training and 'env.close()' at the end, or exit the process.)'''.format(
    ...
    if len(episode_lengths) > 0:
        training_episode_batch = upload_training_episode_batch(data_sources, episode_lengths, episode_rewards,
                                                               episode_types, initial_reset_timestamps, timestamps,
                                                               api_key, env_id=env_id)
    else:
...
def upload_training_episode_batch(data_sources, episode_lengths, episode_rewards, episode_types,
                                  initial_reset_timestamps, timestamps, api_key=None, env_id=None):
...
def upload_training_video(videos, api_key=None, env_id=None):
    ...
    with tempfile.TemporaryFile() as archive_file:
        ...
        logger.info('[%s] Uploading videos of %d training episodes (%d bytes)', env_id, len(videos),
                    util.file_size(archive_file))
        file_upload = resource.FileUpload.create(purpose='video',
    ...
    with tarfile.open(fileobj=archive_file, mode='w:gz') as tar:
        for video_path, metadata_path in videos:
            ...
            if video_name in basenames:
                ...
                raise error.Error(
                    '[{}] Duplicated metadata file name {} in video list: {}'.format(env_id, metadata_name, videos))
            elif not video_name_re.search(video_name):
                raise error.Error(
                    '[{}] Invalid video name {} (must match {})'.format(env_id, video_name, video_name_re.pattern))
            elif not metadata_name_re.search(metadata_name):
                raise error.Error('[{}] Invalid metadata file name {} (must match {})'.format(env_id, metadata_name,
                                                                                              metadata_name_re.pattern))
