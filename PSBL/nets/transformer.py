...
import gin
from PSBL.nets.actor_critic import MLP,DiscreteDist,ContinuousDist
...
class TformerHiddenState:
    ...
    def update(self):
        self.timesteps += 1 # 只有当到了最长的长度 才往回滚一个
...
@gin.configurable(denylist=["state_dim", "obs_dim", "discrete"])
class Decoder_RO(nn.Module):
    def __init__(
        self,
        state_dim: int,
        obs_dim: int,
        ...
        super().__init__()
        d_output_r =  2 * rewards_dim
        d_output_obs = 2 * obs_dim
        self.num_gammas = len(gammas)

        self.r_base = MLP(
            d_inp=int(state_dim/2),
            d_hidden=d_hidden,
            n_layers=n_layers,
            d_output=d_output_r,
            dropout_p=dropout_p,
    ...
    def forward(self, state):
        r_emb, obs_emb = state.chunk(2, dim=-1)
        r_dist_params= self.r_base(r_emb)
        obs_dist_params = self.obs_base(obs_emb)

        return r_dist_params,obs_dist_params,
...
class Decoder_ROA(nn.Module):
    def __init__(
        ...
        super().__init__()
        d_output_latent =  2 * latent_dim
        d_output_act = action_dim if discrete else 2 * action_dim
        self.num_gammas = len(gammas)
        d_output_act *= self.num_gammas
        d_output_r =  2 * rewards_dim
        d_output_obs = 2 * obs_dim
        ...
        self.act_base = MLP(
            ...
            d_hidden=d_hidden,
            n_layers=n_layers,
            d_output=d_output_act,
            dropout_p=dropout_p,
            activation=activation,
        )
        self.r_base = MLP(
            d_inp=int(state_dim/4),
            d_hidden=d_hidden,
            n_layers=n_layers,
            d_output=d_output_r,
            dropout_p=dropout_p,
    ...
    def forward(self, state):
        act_emb, latent_emb = state.chunk(2, dim=-1)
        obs_emb, rewards_emb = latent_emb.chunk(2, dim=-1)
        act_dist_params= self.act_base(act_emb)
        obs_dist_params= self.obs_base(obs_emb)
        r_dist_params= self.r_base(rewards_emb)
        act_dist_params = rearrange(
            act_dist_params, "b ... (g f) -> b ... g f", g=self.num_gammas
        )
        
        return act_dist_params,r_dist_params,obs_dist_params,
        
@gin.configurable(denylist=["state_dim", "action_dim", "discrete"])
class Decoder_ROA_All(nn.Module):
    def __init__(
        self,
        state_dim: int,
        obs_dim:int,
    ...
    def forward(self, state):
        act_emb, latent_emb = state.chunk(2, dim=-1)
        obs_emb, rewards_emb = latent_emb.chunk(2, dim=-1)
        act_dist_params= self.act_base(state)
        obs_dist_params= self.obs_base(obs_emb)
        r_dist_params= self.r_base(rewards_emb)
        act_dist_params = rearrange(
            act_dist_params, "b ... (g f) -> b ... g f", g=self.num_gammas
...
class TransformerOnlineAll(nn.Module):
    def __init__(
        self,
        inp_dim: int,
        max_pos_idx: int,
        ...
        # embedding
        self.position_embedding = nn.Embedding(1*max_pos_idx + 1, embedding_dim=d_model)
        ...
        self.decoder_roa=Decoder_ROA(state_dim=self.d_model,
                                     obs_dim=obs_dim,
                                     action_dim=act_dim,
                                     discrete=discrete,
                                     gammas=gammas,
    ...
    def forward(self, allseq,pos_idxs, hidden_state=None | TformerHiddenState):
        if self.training:
            assert hidden_state is None
        batch, length, dim = allseq.shape #24，312，256 
        h = hidden_state or [[None, None, None] for _ in range(self.n_layers)]

        #pos_idxs=torch.repeat_interleave(pos_idxs,2,dim=1) # 

        # emedding
        pos_emb = self.position_embedding(pos_idxs)
        traj_emb = self.inp(allseq)
        ...
        act_dist_params,r_dist_params,obs_dist_params=self.decoder_roa(traj_emb)
        
        latent_dist_params=traj_emb

        return traj_emb, hidden_state,latent_dist_params,act_dist_params,r_dist_params,obs_dist_params
